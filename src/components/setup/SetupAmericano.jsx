...
function SectionHeader({ children }) {
  return (
    <div className="flex items-center gap-3 mt-7 mb-4">
      <span className="text-xs font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap">{children}</span>
...
function SummaryRow({ label, value, yellow }) {
  return (
    <div className="flex items-center justify-between py-1">
      ...
      <span className={`text-xs font-bold ${yellow ? "text-yellow-400" : "text-gray-200"}`}>{value}</span>
    </div>
  );
}
...
export default function SetupAmericano({ t, code, isAdmin, persist, copyCode, onExitEdit }) {
  ...
  // ── Handlers ──────────────────────────────────────────────────
  function handleName(val)     { setLocalName(val);     clearTimeout(debName.current);     debName.current     = setTimeout(() => persist({ ...t, config: { ...t.config, name: val } }), 600); }
  function handleDesc(val)     { setLocalDesc(val);     clearTimeout(debDesc.current);     debDesc.current     = setTimeout(() => persist({ ...t, config: { ...t.config, description: val } }), 600); }
  function handleLocation(val) { setLocalLocation(val); clearTimeout(debLocation.current); debLocation.current = setTimeout(() => persist({ ...t, config: { ...t.config, location: val } }), 600); }
  function handleDatetime(val) { setLocalDatetime(val); clearTimeout(debDatetime.current); debDatetime.current = setTimeout(() => persist({ ...t, config: { ...t.config, datetime: val } }), 600); }

  function handleRallyCustom(val) {
    setRallyCustom(val);
    clearTimeout(debRally.current);
    const num = parseInt(val);
    if (!isNaN(num) && num > 0) debRally.current = setTimeout(() => persist({ ...t, config: { ...t.config, rallyPoints: num } }), 600);
  }
  function handleGamesCustom(val) {
    setGamesCustom(val);
    clearTimeout(debGames.current);
    const num = parseInt(val);
    if (!isNaN(num) && num > 0) debGames.current = setTimeout(() => persist({ ...t, config: { ...t.config, targetGames: num } }), 600);
  }
  function handleRoundsCustom(val) {
    setRoundsCustom(val);
    clearTimeout(debRounds.current);
    const num = parseInt(val);
    if (!isNaN(num) && num >= 1) debRounds.current = setTimeout(() => { setLocalPrecomputedRounds(null); setLocalRoundWarnings([]); persist({ ...t, config: { ...t.config, maxRounds: num } }); }, 600);
  }
  ...
  function startEdit(i) {
    if (isPairs) { const p = (t.pairInputs || [])[i]; setEditP1(p.p1); setEditP2(p.p2); }
    else         { const p = (t.playerInputs || [])[i]; setEditName(p.name); setEditLvl(p.level); }
    setEditingIdx(i);
  }
  ...
  return (
    ...
    <div className="min-h-screen bg-[#111827] text-gray-50" style={{ fontFamily: "system-ui" }}>
      <div className="max-w-lg mx-auto px-4 pb-16 pt-6">
        ...
        {onExitEdit && (
          <button onClick={onExitEdit} className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-200 font-semibold mb-5 cursor-pointer transition-colors" style={{ background: "none", border: "none", padding: 0 }}>
            ← Volver al torneo
          </button>
        )}
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Código: <span className="text-yellow-400 font-bold tracking-widest">{code}</span></p>
          </div>
          <button
            onClick={handleShare}
            className="text-xs px-3 py-2 rounded-xl border transition-colors cursor-pointer shrink-0 font-semibold"
            style={copied ? { background: "#16a34a20", border: "1px solid #16a34a50", color: "#4ade80" } : { background: "#1f2937", border: "1px solid #374151", color: "#94a3b8" }}
          >
        ...
        {isAdmin && (
          <>
            <div className="flex items-center mb-5">
              <button onClick={() => navigate("/")} className="text-sm text-gray-500 hover:text-gray-300 transition-colors cursor-pointer" style={{ background: "none", border: "none", padding: 0 }}>
                ← Inicio
              </button>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="datetime-local" value={localDatetime} onChange={e => handleDatetime(e.target.value)} className="w-full bg-[#1f2937] border border-gray-700 focus:border-sky-600 rounded-xl text-gray-50 px-3 py-3 text-sm outline-none transition-colors" />
                </div>
                <div>
                  <label className="text-xs text-gray-400 font-semibold block mb-1.5">Ubicación</label>
                  <input value={localLocation} onChange={e => handleLocation(e.target.value)} placeholder="Club, dirección..." className={inputCls} />
                </div>
              </div>
            </div>

            {/* ── 2. Modalidad ── */}
            <SectionHeader>🎮 Modalidad</SectionHeader>
            <div className="space-y-3">
              <div className="flex gap-2">
                {[{ id: "individual", label: "👤 Individual" }, { id: "pairs", label: "👥 Parejas Fijas" }].map(opt => (
                  <button key={opt.id}
                ...
                <div className="bg-[#1f2937] rounded-xl border border-gray-700 overflow-hidden">
                  <div className="flex items-center justify-between p-3">
                    <div>
                      ...
                      <div className="text-xs text-gray-500 mt-0.5">Mezcla jugadores por nivel. Solo visible para el organizador.</div>
                    </div>
                    <button
                      ...
                      style={{ background: useLevels ? "#16a34a30" : "#374151", color: useLevels ? "#4ade80" : "#94a3b8", border: `1px solid ${useLevels ? "#4ade8040" : "transparent"}` }}
                    >{useLevels ? "ON" : "OFF"}</button>
                  </div>
                  {useLevels && (
              ...
              {isPairs
                ? (t.pairInputs || []).map((p, i) => (
                    <div key={i} className="flex items-center gap-3 px-3 py-2.5 bg-[#1f2937] rounded-xl">
                      ...
                      {editingIdx === i ? (
                        <div className="flex-1 flex items-center gap-2 min-w-0" onBlur={e => { if (!e.currentTarget.contains(e.relatedTarget)) commitEdit(i); }}>
                          <input value={editP1} onChange={e => setEditP1(e.target.value)} onKeyDown={e => e.key === "Enter" && commitEdit(i)} autoFocus className="flex-1 min-w-0 text-sm font-medium bg-transparent border-b border-sky-600 outline-none text-gray-50 py-0.5" />
                          <span className="text-gray-600 font-bold shrink-0">/</span>
                          <input value={editP2} onChange={e => setEditP2(e.target.value)} onKeyDown={e => e.key === "Enter" && commitEdit(i)} className="flex-1 min-w-0 text-sm font-medium bg-transparent border-b border-sky-600 outline-none text-gray-50 py-0.5" />
                        </div>
                      ) : (
                        <span className="flex-1 text-sm cursor-pointer hover:underline hover:decoration-dotted" onClick={() => startEdit(i)}>
                          <span className="font-medium">{p.p1}</span><span className="text-gray-600 mx-1.5">/</span><span className="font-medium">{p.p2}</span>
                        </span>
                      )}
                      <button onClick={() => { setEditingIdx(null); persist({ ...t, pairInputs: t.pairInputs.filter((_, idx) => idx !== i) }); }} className="text-gray-600 hover:text-red-400 transition-colors cursor-pointer text-sm leading-none shrink-0">✕</button>
                    </div>
                  ))
                : (t.playerInputs || []).map((p, i) => (
                    <div key={i} className="flex items-center gap-3 px-3 py-2.5 bg-[#1f2937] rounded-xl">
                      <span className="w-6 h-6 rounded-md bg-gray-700 text-gray-400 text-xs font-bold flex items-center justify-center shrink-0">{i + 1}</span>
                      {editingIdx === i ? (
                        <input value={editName} onChange={e => setEditName(e.target.value)} onKeyDown={e => e.key === "Enter" && commitEdit(i)} onBlur={() => commitEdit(i)} autoFocus className="flex-1 text-sm font-medium bg-transparent border-b border-sky-600 outline-none text-gray-50 py-0.5" />
                      ) : (
                        <span className="flex-1 text-sm font-medium cursor-pointer hover:underline hover:decoration-dotted" onClick={() => startEdit(i)}>{p.name}</span>
                      )}
                      {useLevels && (() => {
              ...
              {isPairs ? (
                <div className="space-y-2">
                  <div className="flex gap-2 items-center">
                    <input ref={p1InputRef} value={newP1} onChange={e => setNewP1(e.target.value)} onKeyDown={e => e.key === "Tab" && (e.preventDefault(), p2InputRef.current?.focus())} placeholder="Jugador 1" className={addInputCls} />
                    <span className="text-gray-600 font-bold shrink-0">/</span>
                    <input ref={p2InputRef} value={newP2} onChange={e => setNewP2(e.target.value)} onKeyDown={e => e.key === "Enter" && addPair()} placeholder="Jugador 2" className={addInputCls} />
                  </div>
                  <button onClick={addPair} className="w-full py-2.5 rounded-xl font-bold text-sm cursor-pointer transition-colors" style={{ background: newP1.trim() && newP2.trim() ? COLOR : "#374151", color: "#fff" }}>+ Agregar pareja</button>
                </div>
              ) : (
                <div className="flex gap-2">
                  {useLevels && (
                    <button onClick={() => setNewLvl(l => (l + 1) % 4)} className="w-10 h-10 flex items-center justify-center rounded-xl shrink-0 font-bold text-sm cursor-pointer" style={{ background: "#1f2937", border: `1px solid ${LEVELS[newLvl].color}`, color: LEVELS[newLvl].color }}>
                      {LEVELS[newLvl].short}
                    </button>
                  )}
                  <input ref={nameInputRef} value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key === "Enter" && addPlayer()} placeholder="Nombre del jugador..." className={addInputCls} />
                  <button onClick={addPlayer} className="px-4 py-2.5 rounded-xl font-bold text-sm shrink-0 cursor-pointer transition-colors" style={{ background: newName.trim() ? COLOR : "#374151", color: "#fff" }}>Agregar</button>
                </div>
              )}
            </div>

            {/* Status bar */}
            <div className={`flex items-center px-4 py-2.5 rounded-xl border text-sm font-medium mt-3 ${statusBg} ${statusTxt}`}>
              {statusMsg}
            </div>
              ...
              <div className="mt-3 rounded-xl border border-yellow-500/30 bg-yellow-500/10 px-4 py-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="text-xs text-yellow-300/70 mt-1">El emparejamiento por niveles no tendrá efecto.</div>
                  </div>
                  <button onClick={() => setWarningDismissed(true)} className="text-yellow-500/60 hover:text-yellow-400 text-xs font-bold shrink-0 cursor-pointer">Continuar</button>
                </div>
              </div>
            )}

            {/* ── 4. Pistas ── */}
            <SectionHeader>🏓 Pistas</SectionHeader>
            <div className="flex gap-2">
              {[1,2,3,4,5,6].map(n => (
                <button key={n}
            ...
            <div className="flex flex-col gap-2">
              {SCORING_OPTIONS.map(opt => (
                <button key={opt.id}
                  ...
                  style={{ background: scoring === opt.id ? COLOR + "20" : "#1f2937", border: `1px solid ${scoring === opt.id ? COLOR + "50" : "#374151"}` }}
                >
            ...
            {scoring === "timed" && (
              <div className="bg-[#1f2937] border border-gray-700 rounded-xl p-4 mt-2 space-y-4">
                <div>
                  ...
                  <div className="flex gap-2">
                    {[8,10,12,15,20].map(n => (
                      <button key={n} onClick={() => { setMinutesCustom(""); persist({ ...t, config: { ...t.config, matchMinutes: n } }); }} className="flex-1 py-2 rounded-lg font-bold text-sm cursor-pointer transition-colors" style={{ background: (t.config.matchMinutes ?? 10) === n ? COLOR : "#374151", color: (t.config.matchMinutes ?? 10) === n ? "#fff" : "#94a3b8" }}>{n}</button>
                    ))}
                    <input type="number" value={minutesCustom} onChange={e => handleMinutesCustom(e.target.value)} placeholder="Otro" min={1} max={90} className="w-16 py-2 rounded-lg text-sm text-center font-bold outline-none bg-[#374151] text-gray-300 placeholder:text-gray-600 shrink-0" />
                  </div>
                </div>
                <div>
                  <label className="text-xs text-gray-400 font-semibold block mb-2">¿Qué se anota?</label>
                  <div className="flex gap-2">
                    {[{ id: "games", label: "Games ganados" }, { id: "points", label: "Puntos acumulados" }].map(opt => (
                      <button key={opt.id} onClick={() => persist({ ...t, config: { ...t.config, timedMetric: opt.id } })} className="flex-1 py-2 rounded-lg font-bold text-sm cursor-pointer transition-colors" style={{ background: (t.config.timedMetric ?? "games") === opt.id ? COLOR : "#374151", color: (t.config.timedMetric ?? "games") === opt.id ? "#fff" : "#94a3b8" }}>{opt.label}</button>
                    ))}
                  </div>
                </div>
              </div>
            ...
            {scoring === "rally" && (
              <div className="bg-[#1f2937] border border-gray-700 rounded-xl p-4 mt-2">
                ...
                <div className="flex gap-2">
                  {[16,24,32].map(n => (
                    <button key={n} onClick={() => { setRallyCustom(""); persist({ ...t, config: { ...t.config, rallyPoints: n } }); }} className="flex-1 py-2 rounded-lg font-bold text-sm cursor-pointer transition-colors" style={{ background: (t.config.rallyPoints ?? 24) === n ? COLOR : "#374151", color: (t.config.rallyPoints ?? 24) === n ? "#fff" : "#94a3b8" }}>{n}</button>
                  ))}
                  <input type="number" value={rallyCustom} onChange={e => handleRallyCustom(e.target.value)} placeholder="Otro" min={1} className="w-16 py-2 rounded-lg text-sm text-center font-bold outline-none bg-[#374151] text-gray-300 placeholder:text-gray-600 shrink-0" />
                </div>
              </div>
            )}
            {scoring === "games" && (
              <div className="bg-[#1f2937] border border-gray-700 rounded-xl p-4 mt-2">
                ...
                <div className="flex gap-2">
                  {[4,6,8].map(n => (
                    <button key={n} onClick={() => { setGamesCustom(""); persist({ ...t, config: { ...t.config, targetGames: n } }); }} className="flex-1 py-2 rounded-lg font-bold text-sm cursor-pointer transition-colors" style={{ background: (t.config.targetGames ?? 6) === n ? COLOR : "#374151", color: (t.config.targetGames ?? 6) === n ? "#fff" : "#94a3b8" }}>{n}</button>
                  ))}
                  <input type="number" value={gamesCustom} onChange={e => handleGamesCustom(e.target.value)} placeholder="Otro" min={1} className="w-16 py-2 rounded-lg text-sm text-center font-bold outline-none bg-[#374151] text-gray-300 placeholder:text-gray-600 shrink-0" />
                </div>
              </div>
            )}
            ...
            <div className="mt-7">
              <button onClick={() => setShowAdvanced(s => !s)} className="flex items-center gap-2 text-xs text-gray-500 hover:text-gray-300 font-semibold transition-colors cursor-pointer" style={{ background: "none", border: "none", padding: 0 }}>
                <span>{showAdvanced ? "▾" : "▸"}</span> ⚙️ Configuración avanzada
              </button>
              {showAdvanced && (
                <div className="mt-4 bg-[#1f2937] border border-gray-700 rounded-xl p-4 space-y-5">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <button onClick={() => persist({ ...t, config: { ...t.config, goldenPoint: !(t.config.goldenPoint !== false) } })} className="px-4 py-1.5 rounded-lg text-sm font-bold cursor-pointer shrink-0" style={{ background: t.config.goldenPoint !== false ? "#16a34a30" : "#374151", color: t.config.goldenPoint !== false ? "#4ade80" : "#94a3b8", border: `1px solid ${t.config.goldenPoint !== false ? "#4ade8040" : "transparent"}` }}>
                  ...
                  <div>
                    ...
                    <textarea value={localDesc} onChange={e => handleDesc(e.target.value)} placeholder="Reglas especiales, mensaje a los jugadores..." rows={2} className="w-full bg-[#0f172a] border border-gray-700 focus:border-sky-600 rounded-xl text-gray-50 px-4 py-3 text-sm outline-none transition-colors resize-none" />
                  </div>
                </div>
              )}
            </div>
            ...
            <button
              ...
              style={{ background: ok ? COLOR : "#374151", color: ok ? "#fff" : "#64748b", cursor: ok ? "pointer" : "not-allowed", opacity: ok ? 1 : 0.5 }}
            >
              📋 Revisar y lanzar
            </button>
      ...
      <div className="fixed inset-0 z-50 bg-black/80 flex flex-col">
        <div className="flex-1 flex flex-col max-w-lg mx-auto w-full bg-[#111827] overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-4 border-b border-gray-800 shrink-0">
            <h2 className="text-sm font-black text-gray-50">Revisar antes de iniciar</h2>
            <button onClick={() => setShowLaunchModal(false)} style={{ background: "none", border: "none", padding: "0 4px", cursor: "pointer" }} className="text-gray-500 hover:text-gray-200 text-lg leading-none transition-colors">✕</button>
          </div>
          ...
          {canReshuffle && (
            <div className="flex shrink-0" style={{ borderBottom: "1px solid #1f2937" }}>
              {[{ id: "emparejamientos", label: "🎲 Emparejamientos" }, { id: "resumen", label: "📋 Resumen" }].map(tab => (
                <button
                  key={tab.id}
                  onClick={() => setLaunchModalTab(tab.id)}
                  className="flex-1 py-3 text-xs font-bold transition-colors cursor-pointer"
                  style={{
                    background: "none",
                    borderTop: "none", borderLeft: "none", borderRight: "none",
                    borderBottom: launchModalTab === tab.id ? `2px solid ${COLOR}` : "2px solid transparent",
                    color: launchModalTab === tab.id ? COLOR : "#64748b",
                  }}
                >{tab.label}</button>
              ))}
            </div>
              ...
              <div className="px-4 py-4 space-y-6">
                    ...
                    <div className="space-y-2">
                      {round.courts.map((court, ci) => (
                        <div key={ci} className="bg-[#1f2937] rounded-xl px-3 py-2.5 border border-gray-700">
                          <div className="text-xs text-gray-500 font-semibold mb-1">Pista {ci + 1}</div>
                      ...
                      {round.sittingOut.length > 0 && (
                        <div className="text-xs text-gray-500 px-1">⏳ Descansan: {round.sittingOut.map(p => p.name).join(", ")}</div>
                      )}
                    </div>
                  </div>
                ))}
                {localRoundWarnings.length > 0 && (
                  <div className="rounded-xl border border-yellow-500/20 bg-yellow-500/5 px-3 py-2.5">
                    ...
                    {localRoundWarnings.map((w, wi) => <div key={wi} className="text-xs text-yellow-400/60">{w.message}</div>)}
                  </div>
                )}
              </div>
            )}
            ...
            {(!canReshuffle || launchModalTab === "resumen") && (
              <div className="px-4 py-4 space-y-3">
                ...
                <div className="bg-[#1f2937] border border-gray-700 rounded-xl overflow-hidden">
                  <div className="px-4 py-2.5 border-b border-gray-800"><span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Información</span></div>
                  <div className="px-4 py-3 space-y-1">
                    <SummaryRow label="Nombre" value={t.config.name || "—"} />
                    {t.config.datetime && <SummaryRow label="Fecha" value={new Date(t.config.datetime).toLocaleString("es", { dateStyle: "medium", timeStyle: "short" })} />}
                    {t.config.location && <SummaryRow label="Ubicación" value={t.config.location} />}
                  </div>
                </div>

                <div className="bg-[#1f2937] border border-gray-700 rounded-xl overflow-hidden">
                  <div className="px-4 py-2.5 border-b border-gray-800"><span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Jugadores</span></div>
                  <div className="px-4 py-3 space-y-1">
                    <SummaryRow label="Modalidad" value={isPairs ? "Parejas Fijas" : "Individual"} />
                    <SummaryRow label="Total" value={`${tot} ${isPairs ? "parejas" : "jugadores"}`} />
                    <SummaryRow label="Pistas" value={t.config.courts} />
                    <SummaryRow label="Juegan por ronda" value={Math.min(tot, act)} />
                    {sit > 0 && <SummaryRow label="Descansan por ronda" value={`${sit} ${sit === 1 ? "jugador" : "jugadores"}`} yellow />}
                    {!isPairs && <SummaryRow label="Niveles" value={useLevels ? "Activados" : "Desactivados"} />}
                  </div>
                </div>

                <div className="bg-[#1f2937] border border-gray-700 rounded-xl overflow-hidden">
                  <div className="px-4 py-2.5 border-b border-gray-800"><span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Puntuación</span></div>
                  <div className="px-4 py-3 space-y-1">
                    <SummaryRow label="Sistema" value={scoringLabel} />
                    <SummaryRow label="Detalle" value={scoringDetail} />
                    <SummaryRow label="Punto de oro" value={t.config.goldenPoint !== false ? "Activado" : "Desactivado"} />
                  </div>
                </div>
          ...
          {/* Footer */}
          <div className="px-4 pt-3 pb-5 border-t border-gray-800 shrink-0">
            <p className="text-xs text-gray-600 text-center mb-3">Una vez guardado el primer resultado no podrás modificar la configuración.</p>
            <div className="flex gap-2">
              <button onClick={() => setShowLaunchModal(false)} className="flex-1 py-3 rounded-xl font-bold text-sm text-gray-400 cursor-pointer transition-colors" style={{ background: "#1f2937", border: "1px solid #374151" }}>
                ← Editar
              </button>
              {canReshuffle && launchModalTab === "emparejamientos" && (
                <button onClick={handleReshuffle} className="px-4 py-3 rounded-xl font-bold text-sm text-gray-300 cursor-pointer transition-colors whitespace-nowrap" style={{ background: "#1f2937", border: "1px solid #374151" }}>
                  🔀 Re-sortear
                </button>
              )}
              <button onClick={onStart} className="flex-1 py-3 rounded-xl font-black text-sm text-white cursor-pointer" style={{ background: COLOR }}>
                🎾 Iniciar
              </button>
            </div>
          </div>
